function build_ac_opf!(m::Model)
    ...
    # # Bus variables
    vm = m.ext[:variables][:vm] = @variable(m, [i=N], lower_bound = -0.1, upper_bound = 0.1, base_name = "vm") # voltage magnitude
    ...
    # va_tf_aux = m.ext[:variables][:va_tf] = @variable(m, [i=N_tf], lower_bound = vamin[i], upper_bound = vamax[i], base_name = "va_tf_aux") # voltage angle
    # phi_tf = m.ext[:variables][:phi_tf_aux] = @variable(m, [i=N_tf], lower_bound = -0.1, upper_bound = 0.1, base_name = "phi_tf") # voltage magnitude difference
    ...
    # # m.ext[:constraints][:dadsfaafsd] = @NLconstraint(m, [i=N_tf], vm_tf[i] == 0) # active power i to j
    # # m.ext[:constraints][:etsghsbgh] = @NLconstraint(m, [i=N_tf], va_tf[i] == 0) # active power i to j
    ...
    # # phi_ph_aux = m.ext[:variables][:phi_ph_aux] = @variable(m, [i=N_tf], lower_bound = -0.1, upper_bound = 0.1, base_name = "phi_ph_aux") # voltage magnitude difference

    # # m.ext[:constraints][:dafsd] = @NLconstraint(m, [i=N_tf], vm_ph[i] == 0) # active power i to j
    ...
    # theta_i = m.ext[:variables][:theta_i] = @variable(m, [n = N], base_name = "theta_i") # theta i auxilary variable
...
#     )
    ...
    # m.ext[:constraints][:phi_i_aux_upper1] = @constraint(m, [n in N],
    #     phi_i_aux[n] <=  (vmmax[n] - 1)
    # )

    # m.ext[:constraints][:phi_i_aux_lower1] = @constraint(m, [n in N],
    #     phi_i_aux[n] >=  (vmmin[n] - 1)
    # )


    # m.ext[:constraints][:phi_i_aux_upper2] = @constraint(m, [n in N],
    #     phi_i_aux[n] <=  phi_i[n] - (0)*(vmmin[n] - 1)
    # )

    # m.ext[:constraints][:phi_i_aux_lower2] = @constraint(m, [n in N],
    #     phi_i_aux[n] >=  phi_i[n] - (0)*(vmmax[n] - 1)
    # )
    ...
    # m.ext[:constraints][:va_aux_upper1] = @constraint(m, [n in N],
    #     va_aux[n] <=  (vamax[n])
    # )

    # m.ext[:constraints][:va_aux_lower1] = @constraint(m, [n in N],
    #     va_aux[n] >=  (vamin[n])
    # )


    # m.ext[:constraints][:va_aux_upper2] = @constraint(m, [n in N],
    #     va_aux[n] <=  va[n] - (0)*(vamin[n])
    # )

    # m.ext[:constraints][:va_aux_lower2] = @constraint(m, [n in N],
    #     va_aux[n] >=  va[n] - (0)*(vamax[n])
    # )
...
#     # )
    ...
    # m.ext[:constraints][:pbij] = @NLconstraint(m, [(b,i,j) = B_ac_fr], pb[(b, i, j)] ==  (gb[b] + gfr[b])*(1 + 2 *phi_i_aux[i]) /b_tap[b]^2 - (gb[b] * (1 - 0*(va[i] - va[j] - b_shift[b])^2/2 + phi_i_aux[i] + phi_i[j]))/b_tap[b] - (bb[b] * (va_aux[i] - va[j]))/b_tap[b]) # active power i to j
    # m.ext[:constraints][:qbij] = @NLconstraint(m, [(b,i,j) = B_ac_fr], qb[(b, i, j)] == -(bb[b] + bfr[b])*(1 + 2 *phi_i_aux[i]) /b_tap[b]^2 + (bb[b] * (1 - 0*(va[i] - va[j] - b_shift[b])^2/2 + phi_i_aux[i] + phi_i[j]))/b_tap[b] - (gb[b] * (va_aux[i] - va[j]))/b_tap[b]) # reactive power i to j
    # m.ext[:constraints][:pbji] = @NLconstraint(m, [(b,j,i) = B_ac_to], pb[(b, j, i)] ==  (gb[b] + gto[b])*(1 + 2 *phi_i_aux[j]) - (gb[b] * (1 - 0*(va[j] - va[i] - b_shift[b])^2/2 + phi_i_aux[j] + phi_i[j]))/b_tap[b] - (bb[b] * (va[i] - va_aux[j]))/b_tap[b]) # active power j to i
    # m.ext[:constraints][:qbji] = @NLconstraint(m, [(b,j,i) = B_ac_to], qb[(b, j, i)] == -(bb[b] + bto[b])*(1 + 2 *phi_i_aux[j]) + (bb[b] * (1 - 0*(va[j] - va[i] - b_shift[b])^2/2 + phi_i_aux[j] + phi_i[j]))/b_tap[b] - (gb[b] * (va[i]- va_aux[j]))/b_tap[b]) # reactive power j to i
    ...
    # m.ext[:constraints][:pbij] = @NLconstraint(m, [(b,i,j) = B_ac_fr], pb[(b, i, j)] == - (bb[b] * (va[i] - va[j]))/b_tap[b]) # active power i to j
    # m.ext[:constraints][:qbij] = @NLconstraint(m, [(b,i,j) = B_ac_fr], qb[(b, i, j)] == - (gb[b] * (va[i] - va[j]))/b_tap[b]) # reactive power i to j
    # m.ext[:constraints][:pbji] = @NLconstraint(m, [(b,j,i) = B_ac_to], pb[(b, j, i)] == - (bb[b] * (va[j] - va[i]))/b_tap[b]) # active power j to i
    # m.ext[:constraints][:qbji] = @NLconstraint(m, [(b,j,i) = B_ac_to], qb[(b, j, i)] == - (gb[b] * (va[j] - va[i]))/b_tap[b]) # reactive power j to i
    ...
    # # m.ext[:constraints][:pbdcij] = @constraint(m, [(n,i,j) = B_dc_to], pb_dc[(n, i, j)] ==  (phi_dc[i] - phi_dc[j])/ rd[n]) # active power i to j
    # # m.ext[:constraints][:pbdcji] = @NLconstraint(m, [(n,i,j) = B_dc_fr], pb_dc[(n, i, j)] ==  (phi_dc[i] - phi_dc[j])/ rd[n]) # active power i to j
    ...
    # # momenteel cosunis = 1 genomen, moet nog aangepast worden !!!!!!!!!!

    # m.ext[:constraints][:pbtfacdcij] = @constraint(m, [(n,i,j) = B_tf_ac_dc], pb_tf_ac_dc[(n, i, j)] == (gtf[n] * (ec[n] + 2*phi_i_aux[i])/tc_tf[n]^2 - gtf[n]*(  (1-(va_aux[i] - va_tf[j])) + phi_i_aux[i] + phi_tf[j]) / tc_tf[n] - btf[n]*(va_aux[i] - va_tf[j])  /tc_tf[n])) # active power i to j
    # m.ext[:constraints][:qbtfacdcij] = @constraint(m, [(n,i,j) = B_tf_ac_dc], qb_tf_ac_dc[(n, i, j)] == (-btf[n] * (ec[n] + 2*phi_i_aux[i])/tc_tf[n]^2 + btf[n]*(  (1-(va_aux[i] - va_tf[j])) + phi_i_aux[i] + phi_tf[j]) / tc_tf[n] - gtf[n]*(va_aux[i] - va_tf[j])  /tc_tf[n])) # reactive power i to j
    # m.ext[:constraints][:pbtfdcacji] = @constraint(m, [(n,i,j) = B_tf_dc_ac], pb_tf_dc_ac[(n, i, j)] == (gtf[n] * (ec[n] + 2*phi_i_aux[j]) - gtf[n]*(  (1-(va_aux[j] - va_tf[i])) + phi_i_aux[j] + phi_tf[i]) / tc_tf[n] - btf[n]*(va_tf[i] - va_aux[j])  /tc_tf[n]))  # active power j to i
    # m.ext[:constraints][:qbtfdcacji] = @constraint(m, [(n,i,j) = B_tf_dc_ac], qb_tf_dc_ac[(n, i, j)] == (-btf[n] * (ec[n] + 2*phi_i_aux[j]) + btf[n]*(  (1-(va_aux[j] - va_tf[i])) + phi_i_aux[j] + phi_tf[i]) / tc_tf[n] - gtf[n]*(va_tf[i] - va_aux[j])  /tc_tf[n])) # reactive power j to i
    ...
    # m.ext[:constraints][:pbphacdcij] = @constraint(m, [(n,i,j) = B_tf_ac_dc], pb_ph_ac_dc[(n, i, j)] == (gc[n] * (ec[n] + 2*phi_tf_aux[j]) - gc[n]*(  (1-(va_tf[j] - va_ph[j])) + phi_tf_aux[j] + phi_ph[j])  - bc[n]*(va_tf_aux[j] - va_ph[j])  )) # active power i to j
    # m.ext[:constraints][:qbphacdcij] = @constraint(m, [(n,i,j) = B_tf_ac_dc], qb_ph_ac_dc[(n, i, j)] == (-bc[n] * (ec[n] + 2*phi_tf_aux[j]) + bc[n]*(  (1-(va_tf[j] - va_ph[j])) + phi_tf_aux[j] + phi_ph[j])  - gc[n]*(va_tf_aux[j] - va_ph[j]) )) # reactive power i to j
    # m.ext[:constraints][:pbphdcacji] = @constraint(m, [(n,i,j) = B_tf_dc_ac], pb_ph_dc_ac[(n, i, j)] == (gc[n] * (ec[n] + 2*phi_tf_aux[i]) - gc[n]*(  (1-(va_tf[i] - va_tf[i])) + phi_tf_aux[i] + phi_ph[i])  - bc[n]*(va_ph[i] - va_tf_aux[i])  ))  # active power j to i
    # m.ext[:constraints][:pbphdcacji] = @constraint(m, [(n,i,j) = B_tf_dc_ac], pb_ph_dc_ac[(n, i, j)] == (-bc[n] * (ec[n] + 2*phi_tf_aux[i]) + bc[n]*(  (1-(va_tf[i] - va_tf[i])) + phi_tf_aux[i] + phi_tf[i]) - gc[n]*(va_ph[i] - va_tf_aux[i])  )) # reactive power j to i
    ...
    # # m.ext[:constraints][:pbcvij] = @NLconstraint(m, [(n,i,j) = B_tf_dc_ac_fr], pb_dc_node[i] -  pb_ph[(n,j,i)] == a_loss_cv[n]* ec[n] + b_loss_cv[n] * i_cv[n] + c_loss_cv[n] * i_cv[n]^2) # check als tekens en richtigen goed zijn
    # # m.ext[:constraints][:pqbcvij] = @NLconstraint(m, [(n,i,j) = B_tf_dc_ac_fr], pb_ph[(n,j,i)]^2 +  qb_ph[(n,i,j)]^2 == vm_cv[n]^2 * i_cv[n]^2) # check als tekens en richtigen goed zijn
