function build_ac_opf!(m::Model)
    ...
    ##### Create variables 
    # Bus variables
    vm = m.ext[:variables][:vm] = @variable(m, [i=N], lower_bound = vmmin[i]-1, upper_bound = vmmax[i]-1, base_name = "vm") # voltage magnitude
    ...
    m.ext[:constraints][:pbij] = @constraint(m, [(b,i,j) = B_ac_fr], pb[(b, i, j)] ==  (gb[b] + gfr[b])*(1 + 2 *vm[i]) /b_tap[b]^2 - (gb[b] * (1 - 0*(va[i] - va[j] - b_shift[b])^2/2 + vm[i] + vm[j]))/b_tap[b] - (bb[b] * (va[i] - va[j]))/b_tap[b]) # active power i to j
    m.ext[:constraints][:qbij] = @constraint(m, [(b,i,j) = B_ac_fr], qb[(b, i, j)] == -(bb[b] + bfr[b])*(1 + 2 *vm[i]) /b_tap[b]^2 + (bb[b] * (1 - 0*(va[i] - va[j] - b_shift[b])^2/2 + vm[i] + vm[j]))/b_tap[b] - (gb[b] * (va[i] - va[j]))/b_tap[b]) # reactive power i to j
    m.ext[:constraints][:pbji] = @constraint(m, [(b,j,i) = B_ac_to], pb[(b, j, i)] ==  (gb[b] + gto[b])*(1 + 2 *vm[j]) - (gb[b] * (1 - 0*(va[i] - va[j] - b_shift[b])^2/2 + vm[j] + vm[i]))/b_tap[b] - (bb[b] * (va[j] - va[i]))/b_tap[b]) # active power j to i
    m.ext[:constraints][:qbji] = @constraint(m, [(b,j,i) = B_ac_to], qb[(b, j, i)] == -(bb[b] + bto[b])*(1 + 2 *vm[j]) + (bb[b] * (1 - 0*(va[i] - va[j] - b_shift[b])^2/2 + vm[j] + vm[i]))/b_tap[b] - (gb[b] * (va[j]- va[i]))/b_tap[b]) # reactive power j to i
